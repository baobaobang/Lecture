...
- (void)showFirstImage
{
    ...
    // 原作者的计算思路：目标视图的大小，为scrollView的子控件（SDBrowserImageView）的大小，在layoutSubviews中设置为等于屏幕的大小。有问题
//    CGRect targetTemp = [_scrollView.subviews[self.currentImageIndex] bounds];
    
    // 我的思路：目标视图的大小
    CGFloat width = self.bounds.size.width;
    CGFloat height = (self.bounds.size.width / tempView.image.size.width) * tempView.image.size.height;
    CGFloat y = 0;
    CGFloat x = 0;
    // 如果图片高度大于屏幕高度，图片就居上显示，否则就居中显示
    if (height <= self.bounds.size.height) {
        y = (self.bounds.size.height - height) * 0.5;
    }
    CGRect targetFrame = CGRectMake(x, y, width, height);
    
    // 临时视图的初始frame等于小图的frame
    tempView.frame = rect;
    ...
    [UIView animateWithDuration:SDPhotoBrowserShowImageAnimationDuration animations:^{
        // 将临时视图的初始frame由小图的frame（由小图的image的大小决定）变化到目标视图的frame
